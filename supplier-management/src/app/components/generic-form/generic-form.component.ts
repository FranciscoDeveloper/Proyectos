...
export class GenericFormComponent implements OnInit {
  ...
  editableFields = computed(() => {
    ...
    const excluded = (f: FieldDefinition) =>
      f.type === 'object-list' || f.type === 'dental-chart' || f.type === 'periodontal-chart' || f.isAutoGenerated || f.type === 'entity-select' || !!f.displayOnly;
  ...
  private buildForm() {
    ...
    // Exclude complex/non-input field types and display-only (JOIN-computed) fields
    const fields = schema?.fields.filter(f =>
      f.type !== 'object-list' && f.type !== 'dental-chart' && f.type !== 'periodontal-chart' && !f.isAutoGenerated && !f.displayOnly
    ) ?? [];
