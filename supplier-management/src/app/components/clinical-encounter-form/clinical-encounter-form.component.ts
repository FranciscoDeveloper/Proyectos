...
export class ClinicalEncounterFormComponent implements OnInit {
  ...
  editableFields = computed(() => {
    ...
    return fields.filter(f => !!f.section && ENCOUNTER_SECTIONS.has(f.section) && !excluded(f));
  });
  ...
  /** Returns the section heading when a new section starts, or null otherwise. */
  sectionHeading(field: FieldDefinition, index: number): string | null {
    const fields = this.editableFields();
    if (index === 0 || fields[index - 1].section !== field.section) {
      return ClinicalEncounterFormComponent.SECTION_LABELS[field.section ?? ''] ?? null;
    }
    return null;
  }
