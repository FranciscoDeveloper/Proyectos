 ...
 */
const _stores: Record<string, Record<string, any>[]> = {
...
const _nextId: Record<string, number> = {
  suppliers: 9,
  products: 11,
  payments: 16,
  expenses: 16
...
export function registerEntityHandlers(ipcMain: Electron.IpcMain): void {

  ipcMain.handle('entity:list', (_event, key: string) => {
    return _stores[key] ?? [];
  });

  ipcMain.handle('entity:create', (_event, key: string, data: Record<string, any>) => {
    if (!_stores[key]) _stores[key] = [];
    if (!_nextId[key]) _nextId[key] = 1;
  ...
  ipcMain.handle('entity:update', (_event, key: string, id: number, data: Record<string, any>) => {
    const store = _stores[key];
    if (!store) throw new Error(`Entity '${key}' not found.`);
  ...
  ipcMain.handle('entity:delete', (_event, key: string, id: number) => {
    const store = _stores[key];
    if (!store) throw new Error(`Entity '${key}' not found.`);
    const idx = store.findIndex(r => r['id'] === id);
