...
// ─── Type mirrors (duplicated from Angular models for Node.js context) ─────────

interface SelectOption { value: string; label: string; }
interface FieldDefinition {
  name: string; type: string; label: string;
  required?: boolean; showInList?: boolean; showInDetail?: boolean;
  isTitle?: boolean; isSubtitle?: boolean; isBadge?: boolean;
  options?: SelectOption[]; min?: number; max?: number; step?: number;
  placeholder?: string; pattern?: string; patternMessage?: string;
  minLength?: number; maxLength?: number; sortable?: boolean;
  filterable?: boolean; filterType?: string; format?: string;
  badgeColors?: Record<string, string>;
}
interface EntityMeta {
  key: string; singular: string; plural: string; icon: string;
  description?: string; moduleType?: string;
  disableEdit?: boolean; disableDelete?: boolean;
}
interface EntitySchema { entity: EntityMeta; fields: FieldDefinition[]; }
interface AuthUser { id: number; name: string; email: string; role: string; avatar: string; }
interface AuthResponse { token: string; expiresAt: string; user: AuthUser; schemas: EntitySchema[]; }
...
export function registerAuthHandlers(ipcMain: Electron.IpcMain): void {
  ipcMain.handle('auth:login', (_event, credentials: { email: string; password: string }) => {
